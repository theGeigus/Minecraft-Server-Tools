...
do
    ...
    line="$(tail -3 .server.log)"

    # If player disconnects, check for remaining players
    if [ "$(echo "$line" | grep -o 'Player disconnected:')" == "Player disconnected:" ] || [ "$(echo "$line" | grep -o 'left the game')" == "left the game" ]
    then
    ...
    # If player connects set day and weather cycle to true
	elif [ "$(echo "$line" | grep -o 'Player Spawned:')" == 'Player Spawned:' ] || [ "$(echo "$line" | grep -o 'joined the game')" == "joined the game" ]
	then
